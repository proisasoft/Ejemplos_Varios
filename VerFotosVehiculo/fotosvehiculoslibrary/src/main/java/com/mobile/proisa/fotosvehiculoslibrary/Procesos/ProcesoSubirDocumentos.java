...
public class ProcesoSubirDocumentos extends TareaAsincrona<Uri,Integer,Void> {
    private Vehiculo vehiculo;

    public ProcesoSubirDocumentos(int id, Activity context, OnFinishedProcess listener, Vehiculo vehiculo) {
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        Toast.makeText(getContext(),String.format("Subiendo Imagenes: %d/%d",values[0],values[1]), Toast.LENGTH_LONG).show();
    }
    ...
    protected Void doInBackground(Uri... fotos) {
        ...
        if(connection.isConnected()){
            try {
                PreparedStatement preparedStatement = connection.preparedStatement("DELETE PRBDVEHIDOCUM WHERE NO_DOC=?");
                preparedStatement.setString(1,vehiculo.getId());
                preparedStatement.executeUpdate();


                preparedStatement = connection.preparedStatement("INSERT INTO PRBDVEHIDOCUM (COD_EMPR, COD_SUCU, NO_DOC,IMAGEN, TIPO,CONTADOR) VALUES (1,1,?,?,?,?)");
