package com.mobile.proisa.verfotosvehiculo;
...
public class MainActivity extends AppCompatActivity  implements  VehiculoViewAdapter.OnItemClickListener, OnTapListener<Vehiculo>, TareaAsincrona.OnFinishedProcess, FiltroFragment.SearchListener {
    ...
    public void onItemClick(Vehiculo v, int posicion) {
        Intent intent = new Intent(getApplicationContext(), VerFotosActivity.class);
        ...
        if(!task.hasErrors()){
            switch (task.getId()){
                ...
                case ID_FILTRO_DATOS:
                    ArrayList<Tipo> tipos = task.getData().getParcelableArrayList("tipos");
                    ArrayList<Marca> marcas = task.getData().getParcelableArrayList("marcas");
